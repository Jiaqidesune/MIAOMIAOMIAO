...
namespace Rendering
{
	...
	void Scene::CreateTLAS()
	{
		...
		{
			Bounds3D bbox = meshes[meshInstances[i].meshID]->m_bvh->Bounds();
	...
	void Scene::CreateAccelerationStructures()
	{
		...
		for (int i = 0; i < meshes.size(); i++)
		{
			...
			int numIndices = meshes[i]->m_bvh->GetNumIndices();
			const int* triIndices = meshes[i]->m_bvh->GetIndices();
			...
			verticesUVX.insert(verticesUVX.end(), meshes[i]->m_Vertex.begin(), meshes[i]->m_Vertex.end());
			normalsUVY.insert(normalsUVY.end(), meshes[i]->m_Normal.begin(), meshes[i]->m_Normal.end());

			verticesCnt += meshes[i]->m_Vertex.size();
		}
