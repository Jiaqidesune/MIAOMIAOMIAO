...
inline Matrix4x4 Quat::operator*(const Matrix4x4& m) const
{
	...
	for (int32 I = 0; I < 4; ++I)
	{
		...
		math::VectorQuaternionMultiply(&vt, this, &VQ);
		math::VectorQuaternionMultiply(&vr, &vt, &inv);
		result.m[I][0] = vr.x;
		result.m[I][1] = vr.y;
		result.m[I][2] = vr.z;
		result.m[I][3] = vr.w;
...
inline Quat Quat::operator*=(const Quat& q)
{
	math::VectorQuaternionMultiply(this, this, &q);
	return *this;
}
...
{
	Quat result;
	math::VectorQuaternionMultiply((void*)(&result), (const void*)(this), (const void*)(&q));
	return result;
}
